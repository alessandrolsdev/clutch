...
export async function GET(request: NextRequest, context: RouteContext): Promise<NextResponse> {
  ...
  const backendUrl = new URL(buildApiUrl(`/auth/accounts/${normalizedProvider}/reauth/callback`));
  ...
  try {
    ...
    logServerEvent('error', 'frontend_account_reauth_callback_unreachable', 'Frontend account reauth callback could not reach backend', {
      requestId,
      provider,
      status: 502,
    ...
    return redirectToIntegrations({
      status: 'error',
      message: 'Nao foi possivel concluir a reconexao agora.',
    });
  }
  ...
  if (!backendResponse.ok) {
    return redirectToIntegrations({
      status: 'error',
      message: resolveMessage(payload, 'Nao foi possivel concluir a reconexao agora.'),
    });
  }

  return redirectToIntegrations({
    status: 'success',
    message: resolveMessage(payload, 'Conta reconectada com sucesso.'),
  });
}
